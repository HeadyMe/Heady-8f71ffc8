...
// ═══════════════════════════════════════════════════════════════════════
// COMPARATIVE BENCHMARK APIs
// ═══════════════════════════════════════════════════════════════════════

const { runComparativeBenchmarks, runAgentComparisons, runStrategyComparisons, runMCComparisons, runPublicDomainComparison, runThroughputBenchmarks, runResilienceBenchmarks, AGENT_PROFILES, CHALLENGE_TASKS } = require(path.join(__dirname, "benchmarks", "hc-comparative-benchmarks"));

// Run full comparative benchmark suite
app.post("/api/benchmarks/comparative/run", async (req, res) => {
  try {
    const result = await runComparativeBenchmarks();
    res.json(mcGlobal.enrich(result));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Agent vs Agent comparisons only
app.post("/api/benchmarks/comparative/agents", (req, res) => {
  try {
    const iterations = Math.min(parseInt(req.body.iterations) || 50, 500);
    const result = runAgentComparisons(iterations);
    res.json(mcGlobal.enrich({ type: "agent_comparison", iterations, challenges: result }));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Strategy comparisons only
app.get("/api/benchmarks/comparative/strategies", (req, res) => {
  try {
    const result = runStrategyComparisons();
    res.json(mcGlobal.enrich({ type: "strategy_comparison", strategies: result }));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// MC-augmented decision comparison
app.get("/api/benchmarks/comparative/monte-carlo", (req, res) => {
  try {
    const result = runMCComparisons();
    res.json(mcGlobal.enrich({ type: "mc_comparison", scenarios: result }));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Public domain baseline comparison
app.get("/api/benchmarks/comparative/public-domain", (req, res) => {
  try {
    const result = runPublicDomainComparison();
    res.json(mcGlobal.enrich({ type: "public_domain_comparison", baselines: result }));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.get("/api/benchmarks/comparative/throughput", (req, res) => {
  try {
    const result = runThroughputBenchmarks();
    res.json(mcGlobal.enrich({ type: "throughput_profile", profiles: result }));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.get("/api/benchmarks/comparative/resilience", (req, res) => {
  try {
    const result = runResilienceBenchmarks();
    res.json(mcGlobal.enrich({ type: "resilience_benchmarks", results: result }));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// Agent profiles & challenge task catalog
app.get("/api/benchmarks/comparative/catalog", (req, res) => {
  res.json({
    agents: AGENT_PROFILES,
    challenges: CHALLENGE_TASKS.map(t => ({ id: t.id, name: t.name, category: t.category, applicableAgents: t.applicableAgents })),
    endpoints: {
      fullRun: "POST /api/benchmarks/comparative/run",
      agents: "POST /api/benchmarks/comparative/agents",
      strategies: "GET /api/benchmarks/comparative/strategies",
      monteCarlo: "GET /api/benchmarks/comparative/monte-carlo",
      publicDomain: "GET /api/benchmarks/comparative/public-domain",
      throughput: "GET /api/benchmarks/comparative/throughput",
      resilience: "GET /api/benchmarks/comparative/resilience",
    },
  });
});

// Last comparative results (if available)
app.get("/api/benchmarks/comparative/results", (req, res) => {
  const resultsPath = path.join(__dirname, "benchmarks", "comparative-results.json");
  const data = readJsonFileSafe(resultsPath);
  if (!data) {
    return res.status(404).json({ error: "No comparative results found. Run POST /api/benchmarks/comparative/run first." });
  }
  res.json(mcGlobal.enrich(data));
});
